...
######################################################################
if {!$IS_CHIP} {
    create_clock -name ${AON_SYS_NAME}_clk_rtc32k_aon       -period $CYCLE_32K     -add [get_ports clk_rtc32k_aon]
    create_clock -name ${AON_SYS_NAME}_clk_rstkey_dbnc      -period $CYCLE_32K     -add [get_ports clk_rstkey_dbnc]
    create_clock -name ${AON_SYS_NAME}_clk_xo26m_32k        -period $CYCLE_32K     -add [get_ports clk_xo26m_32k]
    create_clock -name ${AON_SYS_NAME}_clk_30_72m           -period $CYCLE_32K     -add [get_ports clk_30_72m]
    create_clock -name ${AON_SYS_NAME}_clk_3_25m            -period $CYCLE_32K     -add [get_ports clk_3_25m]
    create_clock -name ${AON_SYS_NAME}_V_CLK_32K            -period $CYCLE_32K
    ...
    set CLOCK_GROUP(${AON_SYS_NAME}_V_CLK_32K)              [list ${AON_SYS_NAME}_V_CLK_32K]
...
#####################################################################
    ...
    create_generated_clock -name ${AON_SYS_NAME}_clk_aon_pmu_32k \
                       -master_clock ${AON_SYS_NAME}_clk_rtc32k_aon\
                       -source [get_ports clk_rtc32k_aon]\
                       -divide_by 1 -add \
                       [get_pins ${AON_SYS_HIER}u_aon_sys_top/u_aon_clk_top/u_cksw2_pmu/$CKOUTZ_HIER]

    lappend CLOCK_GROUP(clk_rtc32k_anlg)     [ list ${AON_SYS_NAME}_clk_aon_pmu_32k]
...
#####################################################################
    ...
    create_generated_clock -name ${AON_SYS_NAME}_clk_strappin -add \
                      -master_clock ${AON_SYS_NAME}_clk_aon_pmu_32k \
...
if {$IS_CHIP} {
    ...
    set CLOCK_GROUP(${AON_SYS_NAME}_clk_1_75k)     [ list ${AON_SYS_NAME}_clk_1_875k]
